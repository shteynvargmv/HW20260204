...
Этот проект демонстрирует реализацию аутентификации и авторизации в Spring Boot приложении с использованием Spring Security и JWT (JSON Web Tokens).
...
### Требования
*   Java 17 или выше
*   Maven (встроенный wrapper `mvnw` включен в проект)
...
### 2. Настройка прав доступа (Endpoint Security)
...
### 3. Настройка JWT (Срок действия и Секрет)
Параметры JWT находятся в классе `ru.top.security.jwt.JwtUtil`.

*   **Секретный ключ:** Переменная `SECRET`. Используется для подписи токенов. В продакшене должен быть сложным и храниться в переменных окружения.
*   **Срок действия:** В методе `createToken` (строка `.setExpiration(...)`).
    ```java
    // Пример: 30 минут
    .setExpiration(new Date(System.currentTimeMillis() + 1000 * 60 * 30))
    ```

### 4. Добавление новых контроллеров
Создайте новый класс в пакете `controller`.
...
```
Не забудьте настроить права доступа к новому пути в `SecurityConfig`, иначе по умолчанию он будет требовать просто любой аутентификации.
